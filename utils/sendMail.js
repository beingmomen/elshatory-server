...
exports.getMailForTestimonial = catchAsync(async data => {
  // 1) Render HTML based on a pug template
  const html = await pug.renderFile(
    `${__dirname}/../views/email/testimonial.pug`,
    {
      name: data.name,
      email: data.email,
  ...
  const options = {
    from: data.from, // sender address
    to: data.to, // list of receivers
    subject: `تقيم من ${data.name}`, // Subject line
    text: htmlToText.fromString(html), // plain text body
    html // html body
  };
...
exports.getMailForService = catchAsync(async data => {
  // 1) Render HTML based on a pug template
  const html = await pug.renderFile(`${__dirname}/../views/email/service.pug`, {
    name: data.name,
    phone: data.phone,
    email: data.email,
    description: data.description
  ...
  const options = {
    from: data.from, // sender address
    to: data.to, // list of receivers
    subject: `طلب خدمة من ${data.name}`, // Subject line
    text: htmlToText.fromString(html), // plain text body
    html // html body
  };
